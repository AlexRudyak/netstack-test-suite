...
def test_in_window_data_is_cumulatively_acknowledged(
    ...
    nodeid = "test_in_window_data_is_cumulatively_acknowledged"
...
def test_old_duplicate_data_is_acked_not_reset(
    ...
    conn = established_tcp_connection
    nodeid = "test_old_duplicate_data_is_acked_not_reset"
    ...
    stale = segment(conn, flags="PA", seq=(conn.tracker.seq - 500) & 0xFFFFFFFF, payload=b"stale")
    reply = network_interface.send_receive(stale, timeout=1.5, test_nodeid=nodeid)

    if reply is not None and reply.haslayer(TCP):
        assert not (reply[TCP].flags & RST), "DUT sent RST for an old duplicate segment (§3.10.7.4 requires an ACK)"
...
def test_future_out_of_window_data_does_not_break_connection(
    ...
    conn = established_tcp_connection
    nodeid = "test_future_out_of_window_data_does_not_break_connection"
    expected_ack = conn.tracker.seq

    future = segment(conn, flags="PA", seq=(conn.tracker.seq + 2_000_000) & 0xFFFFFFFF, payload=b"way-ahead")
    reply = network_interface.send_receive(future, timeout=1.5, test_nodeid=nodeid)

    if reply is not None and reply.haslayer(TCP):
        assert not (reply[TCP].flags & RST), "DUT reset the connection for out-of-window data (must just ACK)"
...
def test_segment_with_ack_bit_off_is_silently_dropped(
    ...
    down."""
    conn = established_tcp_connection
    nodeid = "test_segment_with_ack_bit_off_is_silently_dropped"

    reply = network_interface.send_receive(
        segment(conn, flags="P", payload=b"no-ack-flag"), timeout=1.5, test_nodeid=nodeid
    )
    if reply is not None and reply.haslayer(TCP):
        assert not (reply[TCP].flags & RST), "DUT sent RST for a segment with the ACK bit clear (§3.10.7.4: just drop it)"
    assert connection_still_alive(conn, network_interface, nodeid=nodeid)
...
def test_ack_for_unsent_data_does_not_reset(
    ...
    conn = established_tcp_connection
    nodeid = "test_ack_for_unsent_data_does_not_reset"

    bad_ack = segment(conn, flags="A", ack=(conn.tracker.ack + 1_000_000) & 0xFFFFFFFF)
...
def test_keepalive_probe_is_answered_with_current_ack(
    ...
    conn = established_tcp_connection
    nodeid = "test_keepalive_probe_is_answered_with_current_ack"
    expected_ack = conn.tracker.seq

    probe = segment(conn, flags="A", seq=(conn.tracker.seq - 1) & 0xFFFFFFFF)
    ...
    assert reply[TCP].flags & ACK and not (reply[TCP].flags & RST)
    assert reply[TCP].ack == expected_ack, (
